...
function openAny(urls=[]){
  ...
  const u = urls.shift();
  try { window.location.href = u; setTimeout(()=>openAny(urls), 1400); }
  catch(e){ openAny(urls); }
}
...
function render(row){
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
      ...
      <div class="md:col-span-2 space-y-2">
        ...
        <div class="flex gap-2 pt-1">
          <button class="btn" onclick="copy('${(row.activation_code||'').replace(/'/g, '\\\'')}')">Copy Code</button>
          <button class="btn" onclick="copy('${(row.smdp||'').replace(/'/g, '\\\'')}')">Copy SM-DP+</button>
          ${deviceIsAndroid() ? html`<button class="btn" onclick="openAny([
            'intent:#Intent;action=android.settings.ADD_ESIM;end',
          ...
          ])">Open eSIM Setup</button>` : ''}
          ${deviceIsIOS() ? html`<button class="btn" onclick="openAny(['App-Prefs:root=MOBILE_DATA_SETTINGS_ID'])">Open Settings</button>` : ''}
        </div>
  ...
  s2.innerHTML = html`
    ...
    <div class="flex flex-wrap gap-2 mb-2">
      ${deviceIsAndroid() ? html`
        <button class="btn" onclick="openAny([
          'intent:#Intent;action=android.settings.SIM_MANAGEMENT_SETTINGS;end',
          'intent:#Intent;action=android.settings.WIRELESS_SETTINGS;end'
        ])">Open SIM Manager</button>` :
  ...
  s5.innerHTML = html`
    ...
    ${deviceIsAndroid() ? `<button class="btn" onclick="openAny([
      'intent:#Intent;action=android.settings.APN_SETTINGS;end',
      'content://telephony/carriers/preferapn',
      'intent:#Intent;action=android.settings.WIRELESS_SETTINGS;end'
    ])">Open APN Settings</button>` :
...
async function loadData(){
  ...
  if (!participantId) {
    ...
    card.innerHTML = `
      ...
      <p class="text-xs text-slate-500 mt-2">Tip: you can also open a direct link like <span class="kbd">?id=1042</span>. Use <span class="kbd">?csv=</span> to switch data sources.</p>`;
